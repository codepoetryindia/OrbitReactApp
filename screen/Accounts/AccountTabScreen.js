  ...
  componentDidMount() {
    ...
    if (global.tab_name == "Sales") {
      this.setState({ page: global.tab_name }, () => {
        if (this.sales_ref != null) this.sales_ref.onChangeState();
      });
    } else if (global.tab_name == "Expenses") {
      this.setState({ page: global.tab_name }, () => {
        if (this.expenses_ref != null) this.expenses_ref.onChangeState();
      });
    } else {
      if (global.tab_name == "") {
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: Colors.white_color }}>
        {
          <View style={styles.container}>
            <Drawer
              ...
              content={
                <SideMenuComponent
                  isLoadingfunc={(flag) => this.Logout(flag)}
                  goInit={() => console.log("hahah")}
            ...
            >
              {this.state.page == "Reports" && (
                <ReportScreen
                  ...
                  gotoOthertab={(value) => this.gotoOtherScreen(value)}
                />
              )}
              {this.state.page == "Sales" && (
                <SaleScreen
                  ref={(ref) => (this.sales_ref = ref)}
                  navigation={this.props.navigation}
                  showDrawer={() => this.openDrawer()}
                />
              )}
                ...
                <View name="More" style={styles.tab_item}>
                  <BorderlessButton style={styles.tabBarIcon}>
                    <MaterialCommunityIcons
                      ...
                      size={25 * metrics}
                      style={
                        this.state.page == "More" && { color: Colors.red_color }
                      }
                    />
                    {this.state.page == "More" ? (
                      <Text
