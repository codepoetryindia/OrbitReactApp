...
export default class PreviewScreen extends Component {
  ...
  async getUploadsInfo() {
    if (
      (await AsyncStorage.getItem("upload_info")) != "" &&
      typeof (await AsyncStorage.getItem("upload_info")) != "undefined"
    ) {
      this.setState({
        upload_info: JSON.parse(await AsyncStorage.getItem("upload_info")),
      });
    } else {
  ...
  async getBusinessInfo() {
    if (
      (await AsyncStorage.getItem("business_info")) != "" &&
      typeof (await AsyncStorage.getItem("business_info")) != "undefined"
    ) {
      this.setState({
        business_info: JSON.parse(await AsyncStorage.getItem("business_info")),
      });
    } else {
  ...
  async getBioInfo() {
    if (
      (await AsyncStorage.getItem("bio_info")) != "" &&
      typeof (await AsyncStorage.getItem("bio_info")) != "undefined"
    ) {
      this.setState({
        bio_info: JSON.parse(await AsyncStorage.getItem("bio_info")),
      });
    } else {
  ...
  componentDidMount() {
    ...
    this.getUploadsInfo();
    this.getBioInfo();
    if (global.accountType == 1) this.getBusinessInfo();

    this.setState({ isLoading: true });
    UserService.getUserInfo(global.token)
      .then((result) => {
        ...
        if (data.success) {
          ...
          if (data.message == "") alertMessage(ErrorMessage.network_error);
          else alertMessage(ErrorMessage.network_error);
        }
            ...
            <ScrollView style={{ flex: 1 }}>
              ...
              <View style={styles.body}>
                ...
                <View style={styles.item}>
                  ...
                  <Text style={styles.value}>
                    {this.state.user_info.middle_name}
                  </Text>
                </View>
                <View style={styles.item}>
                ...
                <View style={styles.item}>
                  ...
                  <Text style={styles.value}>
                    {changeDatefromServer(this.state.user_info.dob)}
                  </Text>
                </View>
                <View style={styles.item}>
                  <Text style={styles.title}>Account Type</Text>
                  {this.state.user_info.account_type ? (
                    <Text style={styles.value}>
                      {this.state.user_info.account_type.toLowerCase() ==
                      "business"
                        ? "Business"
                        : "Personal"}
                    </Text>
                ...
                <View style={styles.item}>
                  ...
                  <Text style={styles.value}>
                    {this.state.user_info.house_no}
                  </Text>
                </View>
                <View style={styles.item}>
                  <Text style={styles.title}>Address Info</Text>
                  <Text style={styles.value}>
                    {this.state.user_info.address_info}
                  </Text>
                </View>
                <View style={styles.item}>
                  <Text style={styles.title}>Street Name</Text>
                  <Text style={styles.value}>
                    {this.state.user_info.street_name}
                  </Text>
                </View>
                <View style={styles.item}>
                ...
                <View style={styles.item}>
                  ...
                  <Text style={styles.value}>
                    {this.state.user_info.post_code.toUpperCase()}
                  </Text>
                </View>
                <View style={styles.item}>
              ...
              <View style={styles.body}>
                <View style={styles.uploads_item}>
                  <View
                    style={{
                      ...
                      paddingTop: 25 * metrics,
                    }}
                  >
                    <Text style={styles.header}>Identitiy Proof</Text>
                    <Text style={styles.text}>
                      {this.state.upload_info.id_proof_type}
                    </Text>
                  </View>
                  <View style={{ flex: 0.6 }}>
                    <Image
                      source={this.state.upload_info.id_proof}
                      style={styles.img}
                    />
                  </View>
                </View>
                ...
                <View style={styles.uploads_item}>
                  ...
                  >
                    ...
                    <Text style={styles.text}>
                      {this.state.upload_info.address_proof_type}
                    </Text>
                  </View>
                  <View style={{ flex: 0.6 }}>
                    <Image
                      source={this.state.upload_info.address_proof}
                      style={styles.img}
                    />
                  </View>
                </View>
                ...
                <View style={styles.uploads_item}>
                  ...
                  <View style={{ flex: 0.6 }}>
                    <Image
                      source={this.state.bio_info.selfie}
                      style={styles.img}
                    />
                  </View>
                </View>
                ...
                <View style={styles.uploads_item}>
                  ...
                  <View style={{ flex: 0.6 }}>
                    <Image
                      source={this.state.bio_info.video_img}
                      style={styles.img}
                    />
                    <View
                      style={{
                ...
                <>
                  ...
                  <View style={styles.body}>
                    <View style={styles.item}>
                      ...
                      <Text style={styles.value}>
                        {this.state.user_info.company_name}
                      </Text>
                    </View>
                    <View style={styles.item}>
                      <Text style={styles.title}>Co. Number</Text>
                      <Text style={styles.value}>
                        {this.state.user_info.company_number}
                      </Text>
                    </View>
                    <View style={styles.item}>
                      <Text style={styles.title}>Address</Text>
                      <Text style={styles.value}>
                        {this.state.business_info.company_address}
                      </Text>
                    </View>
                    <View style={styles.item}>
                      <Text style={styles.title}>Officer Name</Text>
                      <Text style={styles.value}>
                        {this.state.user_info.account_name}
                      </Text>
                    </View>

                    {this.state.business_info != "" &&
                      this.state.business_info != null && (
                        <View style={styles.item}>
                          <Text style={styles.title}>Appointed Date</Text>
                          {this.state.business_info.officer_list.items.length ==
                          0 ? (
                            <Text style={styles.value} />
                          ) : (
                            <Text style={styles.value}>
                              {changeDatefromServer(
                                this.state.business_info.officer_list.items[0]
                                  .appointed_on
                              )}
                            </Text>
                          )}
                    ...
                    <View style={styles.item}>
                      ...
                      <Text style={styles.value}>
                        {this.state.user_info.company_status}
                      </Text>
                    </View>
                  </View>
                </>
