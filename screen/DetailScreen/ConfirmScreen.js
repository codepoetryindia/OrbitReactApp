...
export default class ConfirmScreen extends Component {
  ...
  nextStep = () => {
    if (global.send_money_invoice) {
      ...
      TransactionService.senMoneyWithInvoice(global.token, global.pay_info)
        .then((res) => {
          var data = res.data.result;
          console.log(
            ".......sendMoneyByBeneficiary...111...." + JSON.stringify(data)
          );
          if (data.success) {
      ...
      this.setState({ isLoading: true });
      TransactionService.sendMoneyByBeneficiary(global.pay_info, global.token)
        .then((res) => {
          var data = res.data.result;
          if (data.success) {
            global.verify_type = "send_money";
            global.transaction_id = data.trn_id;
        ...
        <View style={styles.container}>
          ...
          >
            ...
            <View style={{ width: "90%", flex: 0.8, alignSelf: "center" }}>
              <ImageBackground
                source={Images.doc_img}
                style={{ width: "100%", height: "100%" }}
                resizeMode="stretch"
              >
                <View style={{ flex: 1, flexDirection: "column" }}>
                  <View
                    style={{ flex: 0.4, alignSelf: "center", width: "70%" }}
                  >
                    <View style={{ flex: 0.2 }} />
                    <Text
                      style={{
                    ...
                    <View style={{ flex: 0.4, flexDirection: "row" }}>
                      ...
                      >
                        ...
                        >
                          Account number :{" "}
                          {global.pay_user.user.rb_uk_account_number}
                        </Text>
                        <Text
                          style={{
                            fontFamily: Fonts.adobe_clean,
                      ...
                      >
                        <MaterialIcons name="play-arrow" size={18 * metrics} />
                      </View>
                    </View>
                    <View style={{ flex: 0.2 }} />
                  </View>
                  <Image
                    source={Images.border_line}
                    style={{ width: "90%", alignSelf: "center" }}
                    resizeMode="contain"
                  />
                  <View
                    style={{ flex: 0.6, alignSelf: "center", width: "70%" }}
                  >
                    <View style={{ flex: 0.1 }} />
                    <View
                      style={{
                    ...
                    >
                      ...
                      >
                        ...
                        <Text style={styles.value}>
                          £{" "}
                          {changeNumber(
                            Number(global.pay_user.amount).toFixed(2)
                          )}
                        </Text>
                      </View>
            ...
            >
              <TouchableOpacity
                style={global_style.bottom_active_btn}
                onPress={() => this.pay()}
              >
