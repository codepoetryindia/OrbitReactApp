...
export default class PasswordScreen extends Component {
    ...
    switch (value) {
      case "old":
        if (this.state.old_password.length > 5)
          this.setState({ valid_old_password: true }, () => {
            this.setReady();
          });
        else
          this.setState({ valid_old_password: false }, () => {
            this.setReady();
          });
        break;
      case "new":
        if (this.state.new_password.length > 5) {
          ...
          if (this.state.new_password != this.state.confirm_password) {
            this.setState({ valid_compare: false }, () => {
              this.setReady();
            });
          } else {
            this.setState({ valid_compare: true }, () => {
              this.setReady();
            });
          }
        } else
  ...
  setReady = () => {
    if (
      this.state.old_password.length > 5 &&
      this.state.confirm_password.length > 5 &&
      this.state.new_password.length > 5 &&
  ...
  changePassword = () => {
    ...
    UserService.resetPassword(obj, global.token)
      .then((res) => {
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              ...
              {!this.state.valid_compare &&
                this.state.valid_confirm_password && (
                  <Text style={global_style.error}>
                    {ErrorMessage.mismatch_password}
                  </Text>
                )}
            </View>
