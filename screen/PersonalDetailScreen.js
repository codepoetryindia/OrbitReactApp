...
export default class PersonalDetailScreen extends Component {
  ...
  getNationalityList() {
    ...
    // var token = "&aeT9&G[4}Vaag}R[7*,RA3I9<J88v)pu2m.$l8?k[1wWGfN8043000000001"
    UserService.getNationalityList(global.token)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
          var temp = [];
  ...
  onSubmit = async () => {
    ...
    this.setState({ isLoading: true });
    UserService.signUpPersonalDetail(personalData, global.token)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
          if (global.accountType != 0) {
            ...
            // UserService.signupPostInfo(obj, global.token)
            //   .then((result) => {
  ...
  showMessage = (value) => {
    switch (value) {
      case "post_code":
        ...
        else this.setState({ valid_post_code: false });
        break;
      case "address":
        if (this.state.address != "") this.setState({ valid_address: true });
        else this.setState({ valid_address: false });
        break;
      case "house_no":
        if (this.state.house_no != "") this.setState({ valid_house_no: true });
        else this.setState({ valid_house_no: false });
        break;
      case "building":
        if (this.state.building_name != "")
          this.setState({ valid_building_name: true });
        else this.setState({ valid_building_name: false });
        break;
      case "address_info":
        if (this.state.address_info != "")
          this.setState({ valid_address_info: true });
        else this.setState({ valid_address_info: false });
        break;
      case "street_name":
        if (this.state.street_name != "")
          this.setState({ valid_street_name: true });
        else this.setState({ valid_street_name: false });
        break;
      case "city":
        if (this.state.city != "") this.setState({ valid_city: true });
        else this.setState({ valid_city: false });
        break;
      case "country":
        if (this.state.country != "") this.setState({ valid_country: true });
        else this.setState({ valid_country: false });
        break;
      case "nation":
        if (this.state.nationality != "")
          this.setState({ valid_nationallity: true });
        else this.setState({ valid_nationallity: false });
        break;
      case "county":
        if (this.state.county != "") this.setState({ valid_county: true });
        else this.setState({ valid_county: false });
        break;
    }
  };
  onSearchPostCode = () => {
    ...
    this.setState({ isLoading: true });
    UserService.getAddressInfo(obj)
      .then((res) => {
        var data = res.data.result;
  ...
  selectedNationality = (value) => {
    ...
    this.setState(
      { nationality: this.state.national_list[value].label },
      () => {
        this.checkReady();
      }
    );
                  ...
                  <View style={global_style.input_body}>
                    ...
                    >
                      ...
                      <TouchableOpacity
                        style={
                          this.state.post_code == ""
                            ? global_style.search_btn
                            : global_style.search_move_btn
                        }
                        onPress={() => this.onSearchPostCode()}
                      >
                        <FontistoIcon
                          name="search"
                          size={18 * metrics}
                          color={Colors.gray_color}
                          style={global_style.search_icon}
                        />
                      </TouchableOpacity>
                    </View>
                    ...
                    {!this.state.valid_post_code && (
                      <Text style={global_style.error}>
                        {ErrorMessage.error_post_code_required}
                      </Text>
                    )}
                    ...
                    <View
                      style={
                        this.state.isReady
                          ? global_style.selector_main
                          : global_style.selector_normal
                      }
                    >
                      {this.state.address != "" && (
                        <Text
                    ...
                    {!this.state.valid_address && (
                      <Text style={global_style.error}>
                        {ErrorMessage.error_address_required}
                      </Text>
                    )}
                    ...
                    <View style={{ flexDirection: "row" }}>
                      <View style={{ flex: 0.45 }}>
                        ...
                        {!this.state.valid_house_no && (
                          <Text style={global_style.error}>
                            {ErrorMessage.error_houseno_required}
                          </Text>
                        )}
                      </View>
                      <View style={{ flex: 0.1 }} />
                      <View style={{ flex: 0.45 }}>
                        ...
                        {!this.state.valid_building_name && (
                          <Text style={global_style.error}>
                            {ErrorMessage.error_building_name_required}
                          </Text>
                        )}
                      </View>
                    </View>
                    <TextComponent
                      ...
                      ready={this.state.isReady}
                      onChangeText={(value) =>
                        this.setState({ address_info: value }, () => {
                          this.checkReady();
                        })
                      }
                    >
                    ...
                    {!this.state.valid_address_info && (
                      <Text style={global_style.error}>
                        {ErrorMessage.error_address_info_required}
                      </Text>
                    )}
                    <TextComponent
                    ...
                    {!this.state.valid_street_name && (
                      <Text style={global_style.error}>
                        {ErrorMessage.error_street_name_required}
                      </Text>
                    )}
                    <View style={{ flexDirection: "row" }}>
                      <View style={{ flex: 0.45 }}>
                        ...
                        {!this.state.valid_city && (
                          <Text style={global_style.error}>
                            {ErrorMessage.error_city_required}
                          </Text>
                        )}
                      </View>
                      <View style={{ flex: 0.1 }} />
                      <View style={{ flex: 0.45 }}>
                        ...
                        {!this.state.valid_county && (
                          <Text style={global_style.error}>
                            {ErrorMessage.error_county_required}
                          </Text>
                        )}
                      </View>
                    </View>
