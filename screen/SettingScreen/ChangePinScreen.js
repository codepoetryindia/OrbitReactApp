...
export default class ChangePinScreen extends Component {
  ...
  checkReady = () => {
    if (
      this.state.old_pin_code.length == 5 &&
      this.state.new_pin_code.length == 5
    ) {
  ...
  changePincode = async () => {
    ...
    if (login_info.pin_code != this.state.old_pin_code) {
      ...
      this.setState({ old_pin_code: "", isReady: false }, () =>
        this.old_ref.clean()
      );
      return;
    }
            ...
            >
              <CodePin
                ...
                checkPinCode={(code, callback) =>
                  this.setState({ old_pin_code: code }, () =>
                    callback(code !== "")
                  )
                }
              ...
              <CodePin
                ...
                number={5} // You must pass number prop, it will be used to display 4 (here) inputs
                checkPinCode={(code, callback) =>
                  this.setState({ new_pin_code: code }, () =>
                    callback(code !== "")
                  )
                }
                success={() => this.checkReady()} // If user fill '2018', success is called
