...
export default class CardScreen extends Component {
  ...
  componentDidMount() {
    ...
    CardService.getAllCardList(global.token)
      .then((res) => {
    ...
    analytics().setCurrentScreen("Card", "Card Screen");
  }
  ...
  onChangedFreeze = () => {
    this.setState({ freeze: !this.state.freeze }, () => {
      ...
      if (this.state.freeze) {
        CardService.blockCard(global.token, this.state.select_item.id).then(
          (res) => {
            var data = res.data.result;
            if (data.success) {
        ...
        CardService.unblockCard(global.token, this.state.select_item.id).then(
          (res) => {
            var data = res.data.result;
            if (data.success) {
  ...
  onChangedViewPin = () => {
    ...
    if (this.state.select_item.rb_card_type != "virtual") {
      this.setState({ isLoading: true });
      CardService.getShowCardPin(global.token, this.state.select_item.id)
        .then((res) => {
          var data = res.data.result;
          if (data.success) {
            this.setState({ pin_number: data.response.pin });
          } else {
            if (data.message != "Unauthorized") alertMessage(data.message);
  ...
  activeCard(data) {
    this.setState({ isLoading: true });
    CardService.activeCard(global.token, this.state.select_item.id, data)
      .then((res) => {
        var data = res.data.result;
        console.log("data = ", data);
        if (data.success) {
              ...
              {this.state.isAdd && !this.state.show_carddeatil && (
                <View style={{ marginTop: 60 * metrics }}>
                  <View style={styles.card_view}>
                    ...
                    >
                      <Text style={{ color: "white", fontSize: 18 * metrics }}>
                        Get New Card
                      </Text>
                      ...
                      <View style={{ flexDirection: "row" }}>
                        <View style={styles.typeBody}>
                          <TouchableOpacity
                            style={styles.types}
                            onPress={() => this.goCreateCard("virtual")}
                          >
                            <AntDesign
                              name="earth"
                              size={28 * metrics}
                          ...
                          <Text
                            style={{ marginTop: 5 * metrics, color: "white" }}
                          >
                            Virtual
                          </Text>
                        </View>
                        ...
                        <View style={styles.typeBody}>
                          <TouchableOpacity
                            style={styles.types}
                            onPress={() => this.goCreateCard("physical")}
                          >
                  ...
                  <View style={{ height: 80 * metrics }} />
                  <TouchableOpacity
                    style={{ flexDirection: "column", alignItems: "center" }}
                    onPress={() => this.setState({ isAdd: false })}
                  >
                    <Text style={{ fontSize: 17 * metrics, color: "#000" }}>
                      Already have a card?
                    </Text>
                    <Text
                      style={{
                                                        ...
                                                        <View>
                                                            ...
                                                            <View style={{flexDirection : 'row', flex : 1, marginTop : 5 * metrics, marginBottom : 10 * metrics}}>
                                                                <View style={{flex : 0.3, justifyContent : 'center'}}>
                                                                    <Text style={{fontSize : 18 * metrics, fontFamily : Fonts.adobe_clean,color : 'white',marginLeft : 20 * metrics}}>{this.state.select_item.rb_card_cvv.length == 0 ? '' : this.state.select_item.rb_card_cvv}</Text>
                                                                </View>
                        ...
                        <View style={styles.card_body}>
                          {this.state.select_item.rb_card_type != "virtual" && (
                            <View style={styles.item}>
                              ...
                              {this.state.select_item.rb_card_type ==
                                "virtual" ||
                              this.state.select_item.state == "active" ? (
                                <View
                                  style={{
                          ...
                          <View style={styles.item}>
                            ...
                            >
                              <View style={{ flexDirection: "column" }}>
                                ...
                                >
                                  £ {this.state.select_item.transaction_limit}
                                </Text>
                              </View>
                            </View>
                            <View
                      ...
                      <View style={{ flex: 1, alignItems: "center" }}>
                        ...
                        <View style={styles.card_body}>
                          ...
                          <View style={{ marginTop: 30 * metrics }}>
                            ...
                            <View style={{ marginTop: 10 * metrics }}>
                              <Text style={styles.address_txt}>
                                {this.state.select_item.rb_address_number}{" "}
                                {this.state.select_item.rb_address_street}
                              </Text>
                              <Text style={styles.address_txt}>
                                {this.state.select_item.rb_address_city}
                              </Text>
                              <Text style={styles.address_txt}>
                                {this.state.select_item.rb_address_postal_code}
                              </Text>
                              <Text style={styles.address_txt}>
                                {this.state.select_item.rb_address_iso_country}
                              </Text>
                              <Text style={styles.email_txt}>
                                Email : {global.user_info.email}
                              </Text>
                              <Text style={styles.phone}>
                                Phone : {global.user_info.phone}
                              </Text>
                            </View>
                          </View>
                          <View style={{ marginTop: 40 * metrics }} />
                          <View style={[styles.item, { alignItems: "center" }]}>
                            <TouchableOpacity
                              onPress={() =>
                                this.setState({ show_carddeatil: false })
                              }
                              style={styles.touch_btn}
                            >
                              <MaterialIcons
