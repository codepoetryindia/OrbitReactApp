...
export default class CountryComponent extends Component {
  ...
  componentDidMount() {
    ...
    TransactionService.getTransactionsByCountry(global.token)
      .then((res) => {
          ...
          {this.state.item_arr.map((item, index) => {
            ...
            return (
              <View style={styles.item} key={index}>
                <Flag code={item.country_code} size={32} style={styles.l_img} />
                <TouchableOpacity
                ...
                >
...

                  {
                    <Text style={styles.count} numberOfLines={1}>
                      Send Transactions: {item.send_transaction}
                    </Text>
                  }
                  {
                    <Text style={styles.count} numberOfLines={1}>
                      Receive Transactions: {item.receive_transaction}
                    </Text>
                  }
                ...
                <View style={styles.count_body}>
                  {item.transaction_amount < 0 ? (
                    <Text style={global_style.m_balance} numberOfLines={1}>
                      {" "}
                      - £{" "}
                      {Number(
                        changeNumber(Math.abs(item.transaction_amount))
                      ).toFixed(2)}
                    </Text>
                  ) : (
                    <Text style={global_style.p_balance} numberOfLines={1}>
                      {" "}
                      + £{" "}
                      {Number(
                        changeNumber(Math.abs(item.transaction_amount))
