...
export default class CompanyOfficerScreen extends Component {
  componentDidMount() {
    ...
    CompanyService.getOfficerList(obj, global.token)
      .then((res) => {
  ...
  officerVerification() {
    ...
    CompanyService.verificationOfficer(obj, global.token)
      .then((result) => {
  ...
  selectOfficer = (idx) => {
    ...
    this.setState({ selectedArr: temp }, () => {
      this.checkReady();
    });
  };
                ...
                <View style={global_style.input_body}>
                  <View style={{ flexDirection: "column" }}>
                    {this.state.officer_arr.map((item, idx) => {
                      return (
                        <TouchableOpacity
                          style={{ marginBottom: 10 * metrics }}
                          key={idx}
                          onPress={() => this.selectOfficer(idx)}
                        >
                          <CustomOfficer
                            textName={item.name}
                            textAddress={
