...
// export default class CRMList extends Component {
//     render() {
//         return (
//             <View>
...
export default class CRMList extends Component {
    ...
    getCRMlist(text = '', data={}){
        ...
        InvoiceService.getCRM(obj, global.token).then(res => {
            var data = res.data.result
            console.log('data = ' , data.response.records);
            if (data.success) {
    ...
    FilterData(){
        ...
        this.getCRMlist(this.state.searchText, obj);
        this.closeModal();
    }
        ...
        return (
                                    <View key={idx} style={{flex:1, flexDirection:'row', paddingVertical:5,         borderBottomWidth : 1 ,
                                        borderBottomColor : Colors.white_gray_color}}>
                                            <TouchableOpacity style={styles.item} onPress = {()=>{this.props.navigation.navigate('CRMEdit',{data:item});}}>
    ...
    AddCRM(){
        this.props.navigation.navigate('AddCRM');
    }
    ...
    render() {
        return (
            <SafeAreaView style={{flex:1, height:'100%', width:"100%", position:'relative', flexDirection:'column'}}>
                ...
                <TabHeaderScreen headerTitle="CRM" navigation = {this.props.navigation} showDrawer={() => this.openDrawer()} navigate={this.AddCRM}></TabHeaderScreen>
