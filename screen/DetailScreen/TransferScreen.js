...
export default class TransferScreen extends Component {
  ...
  downloadReceipt() {
    ...
    TransactionService.downlodReceiptFile(global.pay_user.user.id, global.token)
      .then((res) => {
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 0.2, width: "90%", alignSelf: "center" }}>
            <TouchableOpacity onPress={() => this.goBack()}>
              <MaterialCommunityIcons
                name="arrow-left"
                size={28 * metrics}
                style={[global_style.left_arrow, { color: "#000" }]}
              />
            </TouchableOpacity>
          </View>
          <View
          ...
          >
            <Image
              ...
              style={{ width: 65 * metrics, height: 65 * metrics }}
            />
            <View style={{ marginTop: 10 * metrics }} />
            <Text style={{ fontWeight: "500", fontSize: 18 * metrics }}>
              Transfer successful
            </Text>
          </View>
        </View>
        <View style={{ flex: 0.05, backgroundColor: Colors.white_color }} />
        <ScrollView style={{ flex: 0.55 }}>
          <View style={styles.body}>
            ...
            <View style={{ flexDirection: "column", marginTop: 20 * metrics }}>
              ...
              <Text style={styles.value}>
                {global.pay_user.user.rb_uk_sort_code}
              </Text>
            </View>
            <View style={{ flexDirection: "column", marginTop: 20 * metrics }}>
            ...
            <View style={{ flexDirection: "column", marginTop: 20 * metrics }}>
              ...
              <Text style={styles.value}>
                £ {(Number(global.pay_user.amount) + Number(0)).toFixed(2)}
              </Text>
            </View>
            <View style={{ flexDirection: "column", marginTop: 20 * metrics }}>
              <Text style={styles.name}>Date & Time</Text>
              <Text style={styles.value}>
                {changeDatefromAccount(global.pay_user.pay_time.day)}{" "}
                {global.pay_user.pay_time.time}
              </Text>
            </View>
            ...
            >
              <MaterialIcons
                ...
                style={{ marginTop: 10 * metrics }}
              />
              <Text
                style={{
                  color: Colors.main_color,
                  fontSize: 18 * metrics,
                  marginTop: 13 * metrics,
