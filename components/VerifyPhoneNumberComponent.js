...
export default class VerifyPhoneNumberComponent extends Component {
  ...
  resendOTP() {
    ...
    UserService.resendMobilOTP(obj)
      .then((res) => {
        var data = res.data.result;
        if (data.success) alertMessage(ErrorMessage.success_mobile_otp);
        else alertMessage(ErrorMessage.failed_mobile_otp);
  ...
  nextStep() {
    if (
      ...
      UserService.verifyPhoneNumber(obj)
        .then((res) => {
          var data = res.data.result;
          if (data.success) {
            console.log("success");
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={global_style.vbody}>
          ...
          <View style={{ flexDirection: "row", margin: 10 * metrics }}>
            <Text
              style={{ fontSize: 15 * metrics, fontFamily: Fonts.adobe_clean }}
            >
              Don't you receive verify code?
            </Text>
            <TouchableOpacity
