...
export default class UploadProofScreen extends Component {
  ...
  checkReady = () => {
    if (
      ...
      this.state.selectedIdType != -1 &&
      this.state.selectedAddressType != -1
    )
  ...
  setLocalStorage = async () => {
    ...
    UserService.uploadProof(obj, global.token)
      .then((res) => {
  ...
  changeType = (value) => {
    if (value == null) {
      this.setState({ selectedIdType: -1 }, () => {
        this.checkReady();
      });
    } else {
      if (value != 1) {
      ...
      this.setState({ selectedIdType: value }, () => {
        this.checkReady();
      });
    }
  };
  ...
  changeAddresType = (value) => {
    if (value == null) {
      this.setState({ selectedAddressType: -1 }, () => {
        this.checkReady();
      });
    } else {
      this.setState({ selectedAddressType: value }, () => {
        this.checkReady();
      });
    }
  };
    ...
    if (value == 0) {
      ImagePicker.launchCamera(options, (response) => {
        if (response.didCancel) {
          ...
          this.setState({ address_proof: base64_data }, () => {
            this.checkReady();
          });
        }
      });
    ...
    if (value == 0) {
      ImagePicker.launchCamera(options, (response) => {
        if (response.didCancel) {
          ...
          this.setState({ id_proof: response.data }, () => {
            this.checkReady();
          });
        }
      });
      ...
      ImagePicker.launchImageLibrary(options, (response) => {
        if (response.didCancel) {
          ...
          this.setState({ id_proof_img: source }); //show Image
          global.id_proof = source; //new add
          this.setState({ id_proof: base64_data }, () => {
            this.checkReady();
          });
        }
      });
              ...
              <View style={{ flex: 1, flexDirection: "column" }}>
                <View style={{ flex: 0.5, flexDirection: "column" }}>
                  ...
                  {this.state.id_proof != "" ? (
                    <View style={{ flex: 1, flexDirection: "row" }}>
                      <Image
                        source={this.state.id_proof_img}
                        style={styles.imgBtn}
                      />
                      <View style={{ flex: 0.4, flexDirection: "column" }}>
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => this.setState({ id_proof: "" })}
                        >
                          <MaterialIcon
                ...
                <View style={{ flex: 0.5, flexDirection: "column" }}>
                  ...
                  {this.state.address_proof != "" ? (
                    <View style={{ flex: 1, flexDirection: "row" }}>
                      <Image
                        source={this.state.address_proof_img}
                        style={styles.imgBtn}
                      />
                      <View style={{ flex: 0.4, flexDirection: "column" }}>
                        <View style={{ flex: 0.25 }} />
                        <TouchableOpacity
                          style={styles.right_item}
                          onPress={() => this.setState({ address_proof: "" })}
                        >
                          <MaterialIcon
                            name="trash-can-outline"
                            size={30 * metrics}
                        ...
                        <TouchableOpacity
                          style={styles.right_item}
                          onPress={() => this.refreshAddressIDScan()}
                        >
                          <MaterialIcon
                            name="refresh"
                            size={30 * metrics}
                    ...
                    >
                      <View style={{ flex: 0.45, justifyContent: "center" }}>
                        <TouchableOpacity
                          style={styles.btn}
                          onPress={() => this.changeAddressProof(0)}
                        >
                          <MaterialIcon
                            name="camera-outline"
                            size={25 * metrics}
                      ...
                      <View style={{ flex: 0.45, justifyContent: "center" }}>
                        <TouchableOpacity
                          style={styles.btn}
                          onPress={() => this.changeAddressProof(1)}
                        >
                          <MaterialIcon
                            name="image-filter"
                            size={25 * metrics}
