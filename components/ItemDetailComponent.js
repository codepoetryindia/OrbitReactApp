...
export default class ItemDetailComponent extends Component {
  ...
  componentDidMount() {
    ...
    TransactionService.getAllTransactionCategories(global.token)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
          this.setState({
            categories: data.response.records,
            m_categories: data.response.records,
          });
          var count = 0;
          if (data.response.records.length > 0) {
            for (var i = 0; i < data.response.records.length; i++) {
              if (
                this.props.origin_data.rb_classification_account_id[0] ==
                data.response.records[i].id
              ) {
                this.setState({
                  category_name: data.response.records[i].name,
                  selectCatIcon: data.response.records[i].image_variant,
                });
                count++;
                break;
              }
            ...
            if (
              count == 0 &&
              this.props.origin_data.rb_classification_account_id != false
            ) {
  ...
  onHide = (item) => {
    ...
    TransactionService.hideTransactions(obj, global.token)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
  ...
  onShow = (item) => {
    ...
    this.props.funcLoading(true);
    TransactionService.showTransactions(obj, global.token)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
          this.props.closeModal();
  ...
  addAttach = (data) => {
    ...
    TransactionService.addAttach(obj, global.token)
      .then((res) => {
        var data = res.data.result;
  ...
  getAllAttach() {
    ...
    TransactionService.getAttach(this.props.item.naxetra_id, global.token)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
  ...
  addNote = () => {
    ...
    TransactionService.addNote(obj, global.token)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
  ...
  addRating = () => {
    ...
    TransactionService.addRating(obj, global.token)
      .then((res) => {
        var data = res.data.result;
  ...
  async createPDF() {
    if (global.file_path != "") {
      ...
      var url =
        WEB_API + "generate_statement/" + this.props.item.naxetra_id + "/";
      console.log("url = ", url);
      config(options)
        .fetch("POST", url, {
        ...
        .then((response) => {
          ...
          global.file_name =
            "Orbit - " + this.props.origin_data.transaction_number;
          global.trans_id = this.props.item.naxetra_id;
          this.props.navigation.navigate("PdfScreen");
        })
        .catch((err) => {
  ...
  onChangeSearchText(text) {
    this.setState({ search_text: text }, () => {
      if (text == "") {
        ...
        this.setState({ categories: this.state.m_categories }, () =>
  ...
  gotoBack() {
    ...
    TransactionService.updateCategoryInTransaction(
      global.token,
      this.state.selectCatID,
      this.props.item.naxetra_id
    )
      .then((res) => {
        var data = res.data.result;
  ...
  closeBtn(idx) {
    ...
    //this.setState({isLoading : true})
    TransactionService.removeAttach(global.token, id)
      .then((res) => {
        var data = res.data.result;
        if (data.success) {
          this.getAllAttach();
                  ...
                  <View style={styles.item_textBody}>
                    {this.props.origin_data.transaction_type == "out" ? (
                      <Text style={styles.title}>
                        {this.props.origin_data.transaction_info_details}
                      </Text>
                    ) : (
                      <Text style={styles.title}>
                        {!this.props.origin_data.transaction_info_details
                    ...
                    <View style={{ marginTop: 5 * metrics }} />
                    ...
                    {!this.props.origin_data.create_date ? (
                      <Text style={styles.time}>
                        ...
                        {getHoursAndMins(new Date().toISOString())}
                      </Text>
                    ) : (
                      <Text style={styles.time}>
                        {changeDatefromAccount(
                          this.props.origin_data.create_date.split(" ")[0]
                        )}{" "}
                        {getHoursAndMinsFromStr(
                          this.props.origin_data.create_date.split(" ")[1]
                        )}
                      </Text>
                    )}
                    <View style={{ marginTop: 8 * metrics }} />

                    <TouchableOpacity
                      style={styles.rest_button}
                      onPress={() => this.gotoCategories()}
                    >
                      {this.state.selectCatIcon == "" ||
                      !this.state.selectCatIcon ? (
                    ...
                    {this.props.origin_data.transaction_type != "out" ? (
                      ...
                      >
                        + £{" "}
                        {Math.abs(this.props.origin_data.rb_amount).toFixed(2)}
                      </Text>
                    ) : (
                      <Text
                        style={{
                    ...
                    <View>
                      <View style={styles.trans_body}>
                        <View style={styles.mg_body}>
                          ...
                          <TouchableOpacity
                            style={styles.btn}
                            onPress={() => this.customerSupplier()}
                          >
                            <View style={global_style.roundIcon}>
                              <Image
                                source={Images.supplier_img}
                          ...
                          {this.props.origin_data.nx_hide_transaction ==
                          false ? (
                            <TouchableOpacity
                              style={styles.btn}
                              onPress={() => {
                            ...
                            {this.state.receipt_files.map((item, idx) => {
                              return (
                                <View>
                                  <TouchableOpacity
                                    key={idx}
                                    style={{ marginRight: 8 * metrics }}
                                    onPress={() => this.gotoViewPic(item)}
                                  >
                                    <Image
                                      source={{
                                        uri:
                                          "data:image/png;base64," + item.datas,
                                      }}
                                      style={styles.img_item}
                                    />
                                  </TouchableOpacity>
                                  <TouchableOpacity
                                    onPress={() => this.closeBtn(idx)}
                      ...
                      <View style={styles.transaction_detail}>
                        ...
                        >
                          ...
                          <View style={{ flex: 0.5 }}>
                            <Text style={styles.detail_number}>
                              {this.props.origin_data.transaction_number}
                            </Text>
                          </View>
                        </View>
                        <View
                        ...
                        >
                          ...
                          <View style={{ flex: 0.5 }}>
                            <Text style={styles.detail_number}>
                              {!this.props.origin_data.reference
                                ? ""
                                : this.props.origin_data.reference}
                            </Text>
                          </View>
                      ...
                      <View style={styles.transaction_detail}>
                        ...
                        >
                          <View style={{ flex: 0.7 }}>
                            <Text style={styles.detail_title}>
                              {this.props.origin_data.source_type == "bank"
                                ? "Orbit Money"
                                : "Card"}
                            </Text>
                          </View>
                          <View style={{ flex: 0.3 }}>
                            <Text style={styles.detail_value}>
                              {this.props.origin_data.bank_or_card_number}
                            </Text>
                          </View>
                        </View>
                        ...
                        >
                          ...
                          <View style={{ flex: 0.3 }}>
                            <Text style={styles.detail_value}>
                              £ {Number(this.props.item.spend).toFixed(2)}
                            </Text>
                          </View>
                        </View>
                        ...
                        >
                          ...
                          <View style={{ flex: 0.3 }}>
                            <Text style={styles.detail_value}>
                              £ {Number(this.props.item.all_time).toFixed(2)}
                            </Text>
                          </View>
                        </View>
                      ...
                      <View style={styles.rating_view}>
                        ...
                        <View style={{ flex: 0.4 }}>
                          ...
                          />
                          <View style={{ marginBottom: 10 * metrics }} />
                        </View>
                      </View>

                      <View style={styles.border_line} />
                      <View style={{ marginTop: 14 * metrics }} />
                      ...
                      <View style={{ height: 20 * metrics }} />
                    </View>
                  )}
                  {this.state.showNote && (
                    <View style={{ width: "90%", alignSelf: "center" }}>
                      <View style={{ marginTop: 25 * metrics }} />
                      ...
                      />
                      <View style={{ marginTop: 50 * metrics }} />

                      <View style={{ flexDirection: "row" }}>
                        <TouchableOpacity
                          style={global_style.save_button}
                          onPress={() => this.addNote()}
                        >
                          <Text style={styles.text}>Save</Text>
                        </TouchableOpacity>
                        <View style={{ marginLeft: 20 * metrics }} />
                        <TouchableOpacity
                          style={global_style.cancel_button}
        ...
        {this.state.is_subScreen && (
          <View style={styles.category_screen}>
            <View style={styles.back_btn}>
              <TouchableOpacity
                style={{ marginLeft: 10 * metrics }}
                onPress={() => this.setState({ is_subScreen: false })}
              >
                <MaterialCommunityIcons
