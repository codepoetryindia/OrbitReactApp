...
export default class VerfiyNumberScreen extends Component {
  ...
  resendOTP() {
    ...
    TransactionService.transactionVerifyResendOTP(
      global.token,
  ...
  goVerify = () => {
    ...
    if (global.verify_type == "add_beneficiary") {
      ...
      this.setState({ isLoading: true });
      TransactionService.verfiyBeneficiary(
        obj,
      ...
      this.setState({ isLoading: true });
      TransactionService.generatePaymentLink(obj, global.token)
        .then((res) => {
          var data = res.data.result;
          if (data.success) {
      ...
      this.setState({ isLoading: true });
      TransactionService.verifySendMoney(obj, global.token)
        .then((res) => {
          var data = res.data.result;
          if (data.success) {
      ...
      console.log("obj = ", obj);
      OtherService.updateCompanyDetail(global.token, obj)
        .then((res) => {
          var data = res.data.result;
      ...
      OtherService.updatePersonal(global.token, obj)
        .then((res) => {
          var data = res.data.result;
          if (data.success) {
      ...
      <SafeAreaView>
          ...
          <View style={{ flexDirection: "column", width: "80%", flex: 1 }}>
            ...
            <View style={{ flex: 0.7 }}>
              <Text style={styles.sub_title}>
                We are awaiting for the confirmation from the bank for one time
        ...
        {this.state.success && (
          <View style={global_style.black_body}>
            <SuccessComponent
              btnThanks={() => this.gotoScreen()}
              description={ErrorMessage.success_add_beneficiary}
            />
          </View>
        )}
      </SafeAreaView>
