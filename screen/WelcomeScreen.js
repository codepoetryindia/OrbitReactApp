...
export default class WelcomeScreen extends Component {
  ...
  async componentDidMount() {
    ...
    if (global.user_info.middle_name != "") {
      this.setState({
        user_name:
          global.user_info.first_name +
          " " +
          global.user_info.middle_name +
          " " +
      ...
      this.setState({
        user_name:
          global.user_info.first_name + " " + global.user_info.last_name,
      });
    }
  ...
  async nextStep() {
    if (
      typeof global.personal_status == "undefined" ||
      global.personal_status == false
    )
      this.setState({ personal_status: false }, () => {
        this.checkReady();
      });
    else
      this.setState({ personal_status: true }, () => {
        this.checkReady();
      });
    if (global.accountType == 0) {
      ...
      if (!global.business_status) {
        this.setState({ business_status: false }, () => {
          this.checkReady();
        });
      } else
        this.setState({ business_status: true }, () => {
          this.checkReady();
        });
    }
    if (global.proof_status)
      this.setState({ proof_status: true }, () => {
        this.checkReady();
      });
    else
      this.setState({ proof_status: false }, () => {
        this.checkReady();
      });

    if (global.verification_state)
      this.setState({ verification_state: true }, () => {
        this.checkReady();
      });
    else
      this.setState({ verification_state: false }, () => {
        this.checkReady();
      });
  }
  ...
  gotoPersonalDetail() {
    if (this.state.personal_status) return;
  ...
  gotoVerification() {
    if (!this.state.proof_status) return;
    this.props.navigation.navigate("BiometricScreen");
  }

  checkReady() {
    if (
      this.state.personal_status &&
      this.state.proof_status &&
    ...
    return (
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.container}>
          <Image source={Images.small_logo} style={styles.img} />
          ...
          <ScrollView style={styles.body}>
            ...
            <Text style={styles.description}>
              {" "}
              We appreciate the confidence and trust you have placed in us and
              we look forward to helping you in creating bank account with us.{" "}
            </Text>
            <Text style={styles.description2}>
              Please Update the details required in below mentioned sections.
            </Text>
            <View style={styles.control_body}>
              <TouchableOpacity
                onPress={() => this.gotoPersonalDetail()}
                style={
                  this.state.personal_status
              ...
              >
                <View style={styles.button_body}>
                  ...
                  <Text
                    style={
                      this.state.personal_status
                        ? styles.btn_status_text_active
                        : styles.btn_status_text
                    }
                  >
                    {this.state.personal_status ? "Complete" : "Pending"}
                  </Text>
                </View>
              </TouchableOpacity>
              <View style={{ marginTop: 20 * metrics }} />
              {
                ...
                >
                  <View style={styles.button_body}>
                    ...
                    >
                      {this.state.proof_status ? "Complete" : "Pending"}
                    </Text>
                  </View>
                </TouchableOpacity>
              }
              <View style={{ marginTop: 20 * metrics }} />
              {
                ...
                >
                  <View style={styles.button_body}>
                    ...
                    >
                      {this.state.verification_state ? "Complete" : "Pending"}
                    </Text>
                  </View>
                </TouchableOpacity>
              }
              ...
              {global.accountType == 1 && (
                <TouchableOpacity
                  onPress={() => this.gotoBusinessDetail()}
                  style={
                    this.state.business_status
                      ? styles.button_active
                      : styles.button
                ...
                >
                  <View style={styles.button_body}>
                    ...
                    >
                      {this.state.business_status ? "Complete" : "Pending"}
                    </Text>
                  </View>
                </TouchableOpacity>
              )}
