        ...
        <View style={{ flex: 1, justifyContent: "center" }}>
          ...
          <View style={styles.tool_body}>
            <TouchableOpacity
              ...
              onPress={() =>
                this.setState({ contact_type: 0 }, () => this.initData())
              }
            >
              <View style={{ justifyContent: "center" }}>
                <RadioButton
                  value="first"
                  status={
                    this.state.contact_type == 0 ? "checked" : "unchecked"
                  }
                  color={Colors.main_color}
                  onPress={() =>
                    this.setState({ contact_type: 0 }, () => this.initData())
            ...
            <TouchableOpacity
              style={{ flex: 0.5, flexDirection: "row", alignItems: "center" }}
              onPress={() =>
                this.setState({ contact_type: 1 }, () => this.initData())
              }
            >
              <View style={{ justifyContent: "center" }}>
                <RadioButton
                  value="first"
                  status={
                    this.state.contact_type == 1 ? "checked" : "unchecked"
                  }
                  color={Colors.main_color}
                  onPress={() =>
                    this.setState({ contact_type: 1 }, () => this.initData())
              ...
              {this.state.contact_type == 0 ? (
                <View style={{ padding: 20 * metrics }}>
                  ...
                  <TextComponent
                    ...
                    ready={this.state.isReady}
                    onChangeText={(value) =>
                      this.setState({ job_position: value }, () => {
                        this.checkReady();
                      })
                    }
                  />
                  ...
                  <TextComponent
                    ...
                    onChangeText={(value) =>
                      this.setState({ contact_email: value }, () => {
                        ...
                        else this.setState({ exist_email: false });

                        if (
                          this.state.contact_email != "" &&
                          validEmail(this.state.contact_email)
                        )
                          this.setState({ invalid_email: false });
                        else this.setState({ invalid_email: true });
                        this.checkReady();
                      })
                    }
                  />
                  ...
                  />
                  {this.state.contact_phonenumber != "" &&
                    !this.state.isValidContactPhone && (
                      <Text style={global_style.error}>
