...
export default class BusinessDetailScreen extends Component {
  componentDidMount() {
    ...
    if (this.state.activeIdx == 1) {
      ...
      UserService.getBusinessType(objs, global.token)
        .then((result) => {
  ...
  selectType = (value) => {
    this.setState({ b_type: this.state.business_arr[value].txt }, () => {
      this.checkReady();
    });
  };
  ...
  async validate() {
    if (this.state.activeIdx == 2) {
      ...
      UserService.signUpUnregistered(obj, global.token)
        .then((res) => {
          ...
          if (data.success && data.message != "") {
            ...
            } else alertMessage(data.message);
          }
                ...
                >
                  <View style={{ justifyContent: "center" }}>
                    <RadioButton
                      value="first"
                      status={
                        this.state.activeIdx == 1 ? "checked" : "unchecked"
                      }
                      color={Colors.main_color}
                      onPress={() =>
                        this.setState({ isReady: false, activeIdx: 1 })
                      }
                    />
                  </View>
                ...
                >
                  <View style={{ justifyContent: "center" }}>
                    <RadioButton
                      value="second"
                      status={
                        this.state.activeIdx == 2 ? "checked" : "unchecked"
                      }
                      color={Colors.main_color}
                      onPress={() =>
                        this.setState({ isReady: false, activeIdx: 2 })
                      }
                    />
                  </View>
              ...
              {this.state.activeIdx == 2 && (
                ...
                >
                  <Text style={styles.officer}>
                    OFFICER : {global.user_info.first_name}{" "}
                    {global.user_info.middle_name} {global.user_info.last_name}
                  </Text>
