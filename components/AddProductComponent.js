  ...
  componentDidMount() {
    ...
    if (data.length > 0) {
      ...
      for (var i = 0; i < data.length; i++) {
        var obj = {
          label:
            data[i].name + "      (£ " + changeNumber(data[i].lst_price) + ")",
          value: i,
          id: data[i].id,
          color: "black",
        };
      ...
      this.setState(
        {
          ...
          price: this.props.product_arr[value].lst_price,
          amount: (
            this.props.product_arr[value].lst_price * this.state.quantity
          ).toString(),
          product_name: this.props.product_arr[value].name,
          selected_item: value,
        },
                ...
                this.setState(
                  {
                    ...
                    amount:
                      value == ""
                        ? (this.state.price * 1).toString()
                        : (this.state.price * value).toString(),
                  },
                  () => {
                    this.checkReady();
                  }
